...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  const waveformData = useMemo(() => {
    ...
    const random = (i: number) => {
      const x = Math.sin(seed + i * 137.5) * 10000
  ...
  const renderWaveform = useCallback(() => {
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      const isPartiallyFilled = previousBarProgress < progress && barProgress > progress

      if (isPartiallyFilled) {
        // Calculate how much of this bar should be filled
        const fillRatio = (progress - previousBarProgress) / (barProgress - previousBarProgress)
    ...
    ctx.globalAlpha = 1
  }, [waveformData, barWidth, barGap, barRadius, barCount, activeColor, inactiveColor])
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      ...
      <button
        onClick={togglePlay}
        className="transition-colors cursor-pointer text-text hover:text-brand shrink-0"
